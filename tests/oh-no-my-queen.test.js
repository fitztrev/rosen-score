...
import ohNoMyQueen from '../js/goals/oh-no-my-queen.js'
...
describe('test oh-no-my-queen', () => {
    test.each([
        ...
        expect(ohNoMyQueen.checkMoves(chessJsInstance.history({ verbose: true }), finalPosition)).toStrictEqual(color)
    })
})
...
describe('test not oh-no-my-queen', () => {
    test.each([
        ...
        expect(ohNoMyQueen.checkMoves(chessJsInstance.history({ verbose: true }), finalPosition)).toBeFalsy()
    })
})
